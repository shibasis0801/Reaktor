...
// https://docs.oracle.com/javase/7/docs/technotes/guides/jni/spec/types.html
    std::unordered_map<DataType, std::string> typeToJniSignature{
            ...
            {DataType::PromiseType, "Lapp/mehmaan/react/jsi/Promise;"},
            {DataType::FlowType, "Lapp/mehmaan/react/jsi/FlowHandle;"},
            {DataType::SingleArgFn, "Lapp/mehmaan/react/jsi/SingleArgNativeFunction;"}
    };
...
// Darwin selector
    std::string getJavaSignature(DataType returnType,
        ...
        for (auto [_, parameterType] : parameterTypes) {
            if (in(typeToJniSignature, parameterType)) {
        ...
        signature += ")";
        if (in(typeToJniSignature, returnType)) {
